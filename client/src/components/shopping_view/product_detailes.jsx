...
import StarRatingComponent from "../comman/star_rating";
import { addReview, getReviews } from "@/store/shop/review-slice";

function ProductDetailesDialog({ open, setOpen, productDetails }) {
  ...
  function handleAddToCart(getCurrentProductId, getTotalStock) {
    ...
    const quantity = index > -1 ? getCartItems[index].quantity + 1 : 1;
  ...
  const averageReview = reviews && reviews.length > 0
    ? reviews.reduce((sum, reviewItem) => sum + Number(reviewItem.reviewValue), 0) / reviews.length
    : 0;
      ...
      <DialogContent className="grid grid-cols-2 gap-8 sm:p-12 max-w-[90vw] sm:max-w-[80vw] lg:max-w-[70vw]">
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            <p className={`text-3xl text-primary font-bold ${productDetails?.salePrice > 0 ? "line-through" : ""}`}>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-muted-foreground ml-5">{averageReview.toFixed(1)}</span>
          </div>
...
export default ProductDetailesDialog;
